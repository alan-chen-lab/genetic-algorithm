    ...
    for i in range(population_size):
        for j in range(1):
            ans_f2 = g_x_values[i][j] * \
                (1 - math.sqrt(function1_values[i][j] / g_x_values[i][j]))
            # ans_f2 = g_x_values[i][j] * \
            #     (1 - (function1_values[i][j] /
            #      g_x_values[i][j])**2)  # zdt2:nonconvex
...
for iter in range(iteration):  # loop
    ...
    m, n = offspring_array.shape                # 20*30
    gene_number = m*n                           # 600
    # print('gene_number\n = ', gene_number)
