...
class Map():
    ...
    def map_random_obstacle(self, num):    # 生成帶有隨機障礙的map

        self.num = num  # 障礙物數量
        for i in range(self.num):   # 隨機生成障礙物 = 1
            self.data[random.randint(0, self.row-1)
                      ][random.randint(0, self.col-1)] = 1  # a <= N <= b
        if self.data[0][0] == 1:  # 地圖各頂點位置是否為障礙物，是的話改0
            self.data[0][0] = 0
        ...
        for i in range(self.row):  # print map
...
class Population():
    def __init__(self, row, col, num, NP):
        ...
        self.map = self.map_start.map_random_obstacle(self.num)  # 生成帶有障礙的map
        ...
        for i in range(self.NP):  # population number
            ...
            for xk in range(0, self.row):
                ...
                for yk in range(0, self.col):
                    num = (yk) + (xk) * self.col  # 計算點 = x*col + y
...
if __name__ == '__main__':
    ...
    population = Population(row, col, obstacle_number, pop)
    ...
    for i in range(iteration):
        ...
        if popu == []:
            ...
            break
        generation.append(i)
        best_fitness_value.append(np.min(value))  # 計算最小fitness value
        mean_fitness_value.append(np.mean(value))  # 計算平均fitness value
    if popu == []:
        print('no path!')
    else:
        value = path_value(popu, population.col)  # 計算fitness value
        ...
        print('最短路徑值：', minnum)
        print('最短路徑之族群：', popu)
        plt.plot(generation, best_fitness_value, "-r", linewidth=3.0)
        plt.plot(generation, mean_fitness_value, "-b")
        ...
        plt.legend(['best distance value', 'mean distance value'])
        plt.grid(True)
        plt.show()
